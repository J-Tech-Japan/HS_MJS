...
- CheckInfo.cs: 書誌情報の比較結果を格納するデータクラス。旧書誌情報と新書誌情報の差分情報（項番・タイトル・ID・変更種別・表示色）を管理
- CheckForm.cs: 書誌情報チェック結果表示ダイアログフォーム。DataGridViewによる比較結果の表示と、ユーザーによる修正内容の確認・承認機能
- CheckForm.DataGridView.cs: CheckFormのDataGridView操作処理。セルの編集制御、色分け表示、ID候補の自動補完機能
- CheckForm.SetColor.cs: CheckFormの表示色制御処理。変更種別に応じた背景色・文字色の設定と視覚的な差分表示機能
- CheckForm.ExportCsv.cs: 書誌情報比較結果のCSV出力機能。チェック結果をCSVファイルとして保存し、外部ツールでの分析を可能にする
...
- GenerateHTMLButton.Helper.cs: HTML生成で共通利用するヘルパー関数群。パス処理、ファイル操作、例外処理など
...
- GenerateHTMLButton.ProcessImages.cs: HTML出力時の画像処理統合機能。画像抽出・マーカー挿入・ファイルコピー・パス修正を統括し、画像とHTMLコンテンツの整合性を保証
...
### UI・ダイアログフォーム
- Alert.cs: アラート表示用フォーム。アニメーション画像付きの通知ダイアログ
- BookInfo.cs: 書誌情報のデフォルト値入力用ダイアログフォーム。2桁の数値入力と全角・半角変換機能
- CheckForm.cs: 書誌情報チェック結果表示ダイアログフォーム。DataGridViewによる比較結果の表示と、ユーザーによる修正内容の確認・承認機能
- CheckForm.DataGridView.cs: CheckFormのDataGridView操作処理。セルの編集制御、色分け表示、ID候補の自動補完機能
- CheckForm.SetColor.cs: CheckFormの表示色制御処理。変更種別に応じた背景色・文字色の設定と視覚的な差分表示機能
- CheckForm.ExportCsv.cs: 書誌情報比較結果のCSV出力機能。チェック結果をCSVファイルとして保存し、外部ツールでの分析を可能にする
- CoverSelectionForm.cs: 表紙テンプレート選択ダイアログフォーム。EasyCloud・EdgeTracker・汎用パターンなど複数の表紙デザインから選択する機能
- CoverSelectionItem.cs: 表紙選択用のユーザーコントロール。サムネイル画像とキャプション、選択状態を管理するカスタムコントロール
- loader.cs: 処理中表示用のローダーフォーム。バックグラウンド処理中のUI表示制御
- ProgressBar.cs: プログレスバー表示フォーム。長時間処理の進捗状況表示とキャンセル機能付きスレッドセーフな進捗管理
- SetLink.cs: リンク設定ダイアログフォーム。他の項目へのリンク（参照）設定機能
...
- ThisAddIn.cs: VSTOアドインのエントリーポイントクラス。アドインの起動・終了処理とCOMオブジェクトのリソース管理

### UI・システム機能
- RibbonMJS.cs: メインのリボンクラス。書誌情報出力・リンク設定・スタイルチェック・HTML出力の統合制御と共通データ管理
...
### Visual Studio・プロジェクト関連ファイル
- Properties\AssemblyInfo.cs: アセンブリメタデータ（バージョン・著作権・説明など）の定義
- Properties\Resources.Designer.cs: リソースファイル（画像・文字列など）への型安全なアクセスを提供する自動生成クラス
- Properties\Settings.Designer.cs: アプリケーション設定へのアクセス機能を提供する自動生成クラス
- ThisAddIn.Designer.cs: VSTOアドインデザイナーの自動生成コード
- BookInfo.Designer.cs: BookInfoフォームのデザイナー自動生成コード
- CoverSelectionForm.Designer.cs: CoverSelectionFormのデザイナー自動生成コード
- CoverSelectionItem.Designer.cs: CoverSelectionItemコントロールのデザイナー自動生成コード
- CheckForm.Designer.cs: CheckFormのデザイナー自動生成コード
- Alert.Designer.cs: Alertフォームのデザイナー自動生成コード
- loader.Designer.cs: loaderフォームのデザイナー自動生成コード
- ProgressBar.Designer.cs: ProgressBarフォームのデザイナー自動生成コード
- SetLink.Designer.cs: SetLinkフォームのデザイナー自動生成コード
